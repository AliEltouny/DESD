...
class CommunitySerializer(serializers.ModelSerializer):
    ...
    def create(self, validated_data):
        """Override the create method to ensure the creator is set automatically."""
        request = self.context.get('request')
        if request.user.is_authenticated:
            validated_data['creator'] = request.user  # Set the authenticated user as the creator
        else:
            raise serializers.ValidationError("User must be authenticated to create a community.")
